...
description: Use only when the portfolio needs backend, database, auth, API, infra, or dynamic feature design (comments, contact form, lab scores). Design gate before implementation; current deploy is static GitHub Pages, expansion path is Cloudflare.
---
...
The current portfolio is a static Astro site (`output: 'static'` in `astro.config.mjs`) deployed to GitHub Pages. Backend work is future-facing unless the repo already contains backend code.
